locals {
  runner_home_dir = "/usr/local/bin/actions-runner"
  runner_version  = "2.317.0"
  runner_tar      = "actions-runner-linux-${var.architecture == "x86" ? "x64" : "arm64"}-${local.runner_version}.tar.gz"
  default_commands = [
    "sudo yum -y update",
    ...
    "sudo curl -O -L https://github.com/actions/runner/releases/download/v${local.runner_version}/${local.runner_tar}",
